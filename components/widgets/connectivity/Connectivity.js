...
define(function (require) {
  ...
  var Widget = require('../../../geppetto-client/js/components/widgets/Widget');
  var $ = require('jquery');
  var _ = require('underscore');
  var Instance = require('../../../geppetto-client/js/geppettoModel/model/Instance');
  require('../../../geppetto-client/js/components/controls/mixins/bootstrap/modal.js')
  ...
  return Widget.View.extend({
    ...
    linkErev: function (conns, filter) {
      if (this.linkSynapse(conns[0]).length > 0) {
        ...
        var scale = erevs.map(e => {
          if (typeof e.getUnit === 'function' && e.getUnit() === 'V') {
            return 1e3; 
          } else {
            return 1; 
          } 
    ...
    linkGbase: function (conns, filter) {
      if (this.linkSynapse(conns[0]).length > 0) {
        ...
        var scale = gbases.map(g => {
          if (typeof g.getUnit === 'function') {
            return scaleFn(g.getUnit()) 
          } else {
            return 1; 
          } 
        });
    ...
    populateWeights: function (links, filter) {
      ...
      for (var i in links) {
        if (links[i].type.filter(function (t) {
          return this.projectionTypeSummary[filter].indexOf(t) > -1 
        }.bind(this)).length > 0) {
    ...
    createDataFromConnections: function () {
      ...
      if (connectionVariables.length > 0) {
        ...
        this.dataset.populationNodes = Array.from(new Set(this.dataset.nodes.map(x => x.type)))
          .map(function (x,i) {
            return { id: x, type: x, n: populationSizes[i] } 
          });
    ...
    createLayoutSelector: function () {
      ...
      for (var layout in layoutOptions){
    ...
    configViaGUI : function () {
      ...
      var clickHandler = function (event) {
        if (!firstClick){
          ...
          setTimeout(function () {
            firstClick = false;
          }, 200); // closes the window to click again (dbclick)
        } else {
          $('#connectivity-config-modal').modal('hide');
          firstClick = false;
        }
      ...
      setTimeout(function () {
        $('#connectivity-config-modal').find('.card').on('click', clickHandler); 
      }, 200);
    },
    ...
    getView: function (){
      ...
      if (typeof this.nodeColormap.domain === 'function') {
        baseView.nodeColormap = {
          domain: this.nodeColormap.domain(),
