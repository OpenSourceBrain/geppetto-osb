...
define(function (require) {
  ...
  return {
    ...
    linkColormaps: {},
    state: {
      filter: 'projection', colorScale: '{"inh": d3.scaleSequential(d3ScaleChromatic.interpolateBlues), "exc": d3.scaleSequential(d3ScaleChromatic.interpolateReds)}',
    ...
    createMatrixLayout: function (context, state) {
      ...
      var pre = nodes.map((function (x,i) {
        return {
          id: x.id, conn: this.matrix[i].filter(function (d) {
            return d.z; 
          }).length > 0 
        }
      }).bind(this));
      ...
      function addSynapseTypesToLegend (id, colorScale, position) {
        ...
        var legendItem = context.svg.selectAll(id)
          ...
          .attr('transform', function (d, i) {
            ...
            var horz = colorBox.size + position.x + padding.x;
            var vert = i * height + position.y + padding.y;
      ...
      if (this.state.weight) {
        createWeightLegend(this.linkColormaps, this.state.filter);
      }
      ...
      var orderCombo = $('<select/>', { style: 'margin-left: 0.5em;' });
      ...
      var typeCombo = $('<select/>', { style: 'margin-left: 0.5em;' });
      ...
      if (this.state.weight) {
        ...
        var colorCombo = $('<select/>', { style: 'margin-left: 0.5em;' });
